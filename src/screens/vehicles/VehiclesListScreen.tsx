...
type Vehicle = { id: number; model: string; plate: string };
...
export default function VehiclesListScreen() {
  ...
  return (
    <Screen>
      <ThemedButton title="Novo veículo" onPress={() => nav.navigate('VehicleForm')} />
      <FlatList
        style={{ marginTop: 12 }}
        data={data}
        ...
        renderItem={({ item }) => (
          ...
          >
            <ThemedText style={{ fontWeight: '700' }}>{item.model}</ThemedText>
            <ThemedText>Placa: {item.plate}</ThemedText>
            <View style={{ height: 8 }} />
